...
class SolutionRanges(object):
    ...
    def __set__(self, obj, ranges):
        ...
        # modified by Xin @ 16th Dec 2018
        # a, b = ranges
        # if a >= b:
        #         raise ValueError('Wrong range value {}'.format(ranges))
        for rng in ranges:
...
class IndividualBase(object):
    ...
    def __init__(self, power_type, ranges, eps, pi, torque):
        # self.real_time_list = real_time_list
    ...
    def clone(self):
        ...
        '''
        indv = self.__class__(power_type=deepcopy(self.power_type), ranges=deepcopy(self.ranges), \
                    eps=deepcopy(self.eps), pi=deepcopy(self.pi), torque=deepcopy(self.torque))
